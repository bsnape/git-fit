class FastBigFileFinder
  ...
  def fast_find_in_history(number=10)
    ...
    objects = `git verify-pack -v .git/objects/pack/pack-*.idx | grep -v objects | grep -v commit | sort -k4nr`
  ...
  def fast_find_in_working_copy(number=10)
    raise 'no packs found' if @git_lib.get_pack_number == 0
    objects = `git verify-pack -v .git/objects/pack/pack-*.idx | grep -v objects | grep -v commit | sort -k4nr ` #`| head -n #{number}`
